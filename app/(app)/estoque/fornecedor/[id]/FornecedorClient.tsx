...
export function FornecedorClient({
  ...
  return (
    <div className="space-y-6 pb-32">
      ...
      {/* Hero header */}
      <div className="rounded-[24px] bg-white p-6 shadow-premium border-none">
        <div className="flex items-start gap-4">
          {fornecedor.logo_url ? (
            ...
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-surface text-xl font-bold text-forest">
              {iniciais(nome)}
            </div>
          )}
          <div className="min-w-0 flex-1 pt-1">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {fornecedor.tipo && (
                <span className="rounded-full bg-gray-50 px-2.5 py-1 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                  {TIPO_FORNECEDOR_LABEL[fornecedor.tipo]}
                </span>
              )}
              {typeof fornecedor.prazo_entrega_dias === "number" && (
                <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-2.5 py-1 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
      ...
      {/* Tabs */}
      <div className="flex p-1 bg-white rounded-full shadow-premium gap-1">
        {(
          [
            ["produtos", "Produtos"],
      ...
      {tab === "produtos" && (
        <div className="space-y-3">
          {produtos.length === 0 ? (
            <div className="rounded-[24px] bg-white p-10 text-center shadow-premium">
               <Package className="h-10 w-10 text-gray-200 mx-auto mb-4" />
               <p className="text-sm font-bold text-gray-900">Nenhum produto</p>
               <p className="text-xs text-gray-500 mt-1">Este fornecedor ainda não cadastrou produtos.</p>
            </div>
          ) : (
            produtos.map((p) => (
        ...
        <div className="space-y-3">
          {avaliacoes.length === 0 ? (
            <div className="rounded-[24px] bg-white p-10 text-center shadow-premium">
               ...
               <p className="text-sm font-bold text-gray-900 mt-4">Sem avaliações</p>
               <p className="text-xs text-gray-500 mt-1">Ainda não há avaliações para este fornecedor.</p>
            </div>
          ) : (
            avaliacoes.map((a) => (
              <div
                key={a.id}
                className="rounded-[20px] bg-white p-5 shadow-premium"
              >
...
function ProdutoCard({
  ...
  return (
    <div className="rounded-[20px] bg-white p-5 shadow-premium border-none">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <DoseBadge mg={produto.dose_mg} className="bg-forest" />
          <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-50 px-2.5 py-1 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        <button
          onClick={onPedir}
          disabled={esgotado}
          className="rounded-full bg-forest px-6 py-2.5 text-sm font-bold text-white transition-all active:scale-95 disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400 shadow-lg shadow-forest/20"
        >
          Pedir agora
        </button>
      </div>
...
function ConfirmarPedidoModal({
  ...
  async function confirmar() {
    ...
    try {
      ...
      // Notify the supplier (best-effort).
      if (fornecedor.user_id) {
        await sendPush(
          fornecedor.user_id,
          pedidoNotificacoes.novoPedidoParaFornecedor(codigo),
        );
      }
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center">
      ...
      <div className="relative max-h-[92vh] w-full max-w-md overflow-y-auto rounded-t-[32px] bg-bg p-8 shadow-2xl animate-slide-up z-[101]">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="rounded-full bg-white p-2 text-gray-400 shadow-sm"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {/* Resumo */}
        <div className="rounded-[24px] bg-white p-6 shadow-premium">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <DoseBadge mg={produto.dose_mg} className="bg-forest" />
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className="h-10 w-10 rounded-full bg-gray-50 flex items-center justify-center text-lg font-bold text-gray-600 transition-all active:scale-90"
              >
                −
              </button>
              <span className="w-4 text-center text-lg font-black text-gray-900">
                {quantidade}
              </span>
              <button
        ...
        <div className="mt-8">
          <p className="mb-4 flex items-center gap-2 text-sm font-bold text-gray-900">
            <MapPin size={16} className="text-forest" strokeWidth={2.5} /> Endereço de entrega
          </p>
        ...
        <button
          onClick={confirmar}
          disabled={loading}
          className="mt-8 flex w-full items-center justify-center gap-2 rounded-full bg-forest py-4.5 text-base font-bold text-white shadow-lg shadow-forest/30 transition-all active:scale-[0.98] disabled:opacity-60"
        >
...
function SucessoModal({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6">
      ...
      <div className="animate-fab-pop relative w-full max-w-sm rounded-[32px] bg-white p-8 text-center shadow-2xl">
        <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-surface">
          <CheckCircle2 className="h-12 w-12 text-forest" strokeWidth={2.5} />
        </div>
        ...
        <div className="mt-6 rounded-2xl bg-gray-50 py-4">
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
            Código do pedido
          </p>
          <p className="text-xl font-black text-forest tracking-tighter">{codigo}</p>
        </div>
        <div className="mt-8 flex flex-col gap-3">
          <Link
            href="/meus-pedidos"
            className="flex items-center justify-center gap-2 rounded-full bg-forest py-4 text-sm font-bold text-white shadow-lg shadow-forest/20 active:scale-[0.98]"
          >
            <ShoppingBag size={18} /> Ver meus pedidos
          </Link>
          <button
            onClick={onClose}
            className="rounded-full py-3 text-sm font-bold text-gray-400 hover:text-gray-600 transition-colors"
          >
            Continuar comprando
          </button>
        </div>
